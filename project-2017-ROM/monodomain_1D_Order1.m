...
%% THIS HAS NEVER BEEN USED FOR THE PHD THESIS AND IS COPIED FROM "COMPUATATIONAL METHODS IN BIOMECHANICS" ASSINGMENT 3 %%% 
  ...
  print(strcat('plot_',num2str(time_step_pde),'_n_',num2str(n_elem),'_O1'),'-dpng');
  
  figure(99);
  tt = linspace(0,t_end,num_of_steps_pde);
  plot(tt, V_m_out);
  %savefig(strcat('profile',num2str(time_step_pde),'_O1.fig'));
  print(strcat('profile_',num2str(time_step_pde),'_n_',num2str(n_elem),'_O1'),'-dpng');
...
outfile=strcat('out','Vm_time_space','_dt_',num2str(timestep),'_n_',num2str(n_elem),'_O1.csv');
csvwrite(outfile,V_m_time); 
end


function[]=OutNodeinTime(Vm_time,indx,timestep,n_elem)
outfile=fopen(strcat('out_Vm_node_',num2str(indx),'_dt_',num2str(timestep),'_n_',num2str(n_elem),'_O1.txt'),'w');
fprintf(outfile,'%6.6f\n',Vm_time(:,indx));
fclose(outfile);
end
