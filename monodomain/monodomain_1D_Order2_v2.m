...
%% THIS HAS NEVER BEEN USED FOR THE PHD THESIS AND IS COPIED FROM "COMPUATATIONAL METHODS IN BIOMECHANICS" ASSINGMENT 3 %%% 
  ...
  %fast-twitch
  if(n_elem>12)
      I_stim=75*(n_elem/12); %the reference case must be adjusted by experimenting
  else
      I_stim=75;
  end
  ...
  for time = 1:num_of_steps_pde   
    ...
    %----------------------------------------------------------------------
    % PARABOLIC EQUATION
    bb=setRHS(method,V_m,time_step_pde/2,dx,D);
    ...
    %integrate over the ODE
    tspan = (time-1):1.0/ (num_of_steps_ode):time;
    tspan=tspan*time_step_pde;
  ...
  savefig(strcat('monodomain_',num2str(time_step_pde),'_O2_v2.fig'));
  print(strcat('monodomain_',num2str(time_step_pde),'_O2_v2'),'-dpng');
  
  figure(99);
  tt = linspace(0,t_end,num_of_steps_pde);
  plot(tt, V_m_out);
  savefig(strcat('profile',num2str(time_step_pde),'_O2_v2.fig'));
  print(strcat('profile',num2str(time_step_pde),'_O2_v2'),'-dpng');
...
outfile=fopen(strcat('out','Vm_',num2str(timestep),'_',num2str(n_elem),'_O2_v2.txt'),'w');
