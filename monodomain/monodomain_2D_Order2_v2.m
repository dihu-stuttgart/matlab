%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = monodomain_2D_Order2_2()
...
%% THIS HAS NEVER BEEN USED FOR THE PHD THESIS AND IS COPIED FROM "COMPUATATIONAL METHODS IN BIOMECHANICS" ASSINGMENT 3 %%% 
  ...
  t_end_stimulation = 0.5;
   ...
   %fast-twitch
  if(n_elem>4)
      I_stim=75*(n_elem/4); %the reference case must be adjusted by experimenting
  else
      I_stim=75;
  end
  ...
  KK= StiffnessMatrix_2D(method,num_of_points,time_step_pde/2,dx,D); 
  ...
  for time = 1:num_of_steps_pde 
    ...
    %----------------------------------------------------------------------
    % PARABOLIC EQUATION
    bb=setRHS_2D(method,V_m,time_step_pde/2,dx,D);
